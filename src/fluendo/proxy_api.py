...
def customers_proxy(request):
    ...
    if r.status_code == 200:
        if type(r.json()) == dict:
            ...
            request_data = r.json()
        data = sorted(request_data, key=operator.itemgetter("company_name"))
    else:
        data = None
...
def company_proxy(request):
    ...
    if r.status_code == 200:
        if type(r.json()) == dict:
            ...
            request_data = r.json()
        data = sorted(request_data, key=operator.itemgetter("name"))
    else:
        data = None
    return JsonResponse(data, safe=False)
...
def contacts_proxy_post(data):
    web_auth = {
        "Authorization": "Token " + settings.WEB_FLUENDO_TOKEN,
