...
class HierarchyForm(forms.Form):
    ...
    issues = forms.MultipleChoiceField(
        widget=forms.SelectMultiple(
            attrs={"class": "select selector-ui", "multiple": "multiple"}
        ),
        required=False,
        choices=[],
    )
    field = forms.ChoiceField(
        widget=forms.Select(attrs={"class": "select"}), required=False, choices=[]
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["issues"].choices = [
            (x.id, x.name) for x in self.hierarchy_service.addon_jira.issue_types()
        ]
        self.fields["field"].choices = [
            (x["key"], x["name"]) for x in self.hierarchy_service.addon_jira.fields()
        ] + [("", "None")]
        self.fields["link"].choices = [
            (x.id, x.name) for x in self.hierarchy_service.addon_jira.issue_link_types()
        ] + [("", "None")]
