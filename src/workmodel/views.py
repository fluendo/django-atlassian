...
from django_atlassian.decorators import jwt_qsh_exempt, jwt_required
...
def initiative_status(request):
    ...
    sc = SecurityContext.objects.filter(
        host=jira_host, key=request.atlassian_sc.key, product_type="jira"
    ).get()
    wm = WorkmodelService(sc)
    issues = wm.hierarchy.child_issues(key)
    summary = get_issues_progress(issues)
    return render(request, "workmodel/initiative_status.html", summary)
    ...
    for i in issues:
        for v in i.fields.fixVersions:
            if not v in versions:
                ...
                url = "{}?issueKey={}&fixVersion={}".format(
                    reverse("workmodel-issue-versions-progress"), key, v.name
                )
                token = atlassian_jwt.encode_token(
                    "GET", url, sc.client_key, sc.shared_secret
                )
                versions[v.name]["progress"] = "{}://{}{}&jwt={}".format(
                    request.scheme, request.get_host(), url, token
                )
            p = {
                "project": i.fields.project.key,
                "url": "{0}/browse/{1}/fixforversion/{2}".format(
                    jira_host, i.fields.project.key, v.id
                ),
                "status": "released" if v.released else "unreleased",
            }
            versions[v.name]["projects"] = versions[v.name]["projects"] + [p]
    return render(request, "workmodel/issue_versions.html", {"data": versions})
...
def issue_versions_progress(request):
    ...
    wm = WorkmodelService(sc)
    issues = wm.hierarchy.child_issues(
        key, extra_jql="AND fixVersion IN ('{0}')".format(version)
    )
    summary = get_issues_progress(issues)
    return render(request, "workmodel/initiative_status.html", summary)
...
def addon_enabled(request):
    ...
    for project in projects:
        ...
        # Update the options in case the project name or key have changed
            ...
            option = [
                option for option in options if option.properties.id == project.id
            ][0]
    ...
    if not PeriodicTask.objects.filter(args=json.dumps([sc.id])).count():
        PeriodicTask.objects.create(
            interval=schedule,
            name="Update In-Progress business time",
...
def project_updated(request):
    ...
    options = j.custom_field_options_app(field_name)
    option = [
        option for option in options if option.properties.id == str(project["id"])
    ][0]
    option.update(value=project["name"], id=option.id, properties=properties)
    return HttpResponse(204)
...
def issue_updated(request):
    ...
    for item in changelog["items"]:
        ...
        if item["field"] == "status":
            ...
            from_issue = parse.parse(
                "This issue is contained in {}", item["fromString"]
            )
            if from_issue:
                # Update other
...
def business_time_transitions_dashboard_item_configuration(request):
    ...
    if filter_id:
        # Save the property
        wm.jira.create_dashboard_item_property(
            dashboard_id, item_id, "business-time-transitions-configuration", filter_id
        )
        # Redirect to render
...
def business_time_transitions_dashboard_item(request):
    ...
    try:
        conf = wm.jira.dashboard_item_property(
            dashboard_id, item_id, "business-time-transitions-configuration"
        )
    except:
        filters = wm.jira.search_filters_gen()
        # Redirect to conf
    ...
    for m in range(1, 13):
        ...
        name = datetime.datetime.combine(
            day_first, datetime.datetime.min.time()
        ).strftime("%b")
        month = {
            "name": name,
...
def business_time_configuration(request):
    ...
    fields = [
        f
        for f in wm.jira.fields()
        if "schema" in f and f["schema"]["type"] in ["date", "datetime"]
    ]
    fields.sort(key=lambda field: field["name"])
    return render(
        request,
        "workmodel/business_time_configuration.html",
        {
            "conf": conf,
            "fields": fields,
        },
...
def hierarchy_configuration(request, id):
    ...
    # Create the formset
    HierarchyFormSet = formset_factory(
        HierarchyForm, can_order=True, can_delete=True, extra=0
    )
    formset = HierarchyFormSet(
        form_kwargs={"hierarchy_service": wm.hierarchy}, initial=h["levels"]
    )
    return render(
        request,
        "workmodel/hierarchy_configuration.html",
        {
            "formset": formset,
            "id": id,
        },
...
def hierarchy_update_configuration(request, id):
    ...
    formset = HierarchyFormSet(
        request.GET, form_kwargs={"hierarchy_service": wm.hierarchy}
    )
