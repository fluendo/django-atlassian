...
from workmodel.hierarchy import (
    CustomHierarchyLevel,
    EpicHierarchyLevel,
    Hierarchy,
    HierarchyLevel,
    SubTaskHierarchyLevel,
)
from workmodel.transitions import Transition, Transitions, TransitionsCollection
...
class JiraService(object):
    def __init__(
        self, sc, jira=None, addon_jira=None, account_id=None, *args, **kwargs
    ):
        self.sc = sc
        if jira:
            self.jira = jira
    ...
    def _connect_jira(self, account_id):
        if not account_id:
            jira = WorkmodelJira(
                self.sc.host,
                jwt={"secret": self.sc.shared_secret, "payload": {"iss": self.sc.key}},
            )
        else:
...
class WorkmodelService(JiraService):
    ...
    def __init__(self, sc, *args, **kwargs):
        ...
        self.business_time = BusinessTimeService(
            self.sc, self.hierarchy, *args, **kwargs
        )
    ...
    def configuration_2(self):
        conf = self.addon_jira.app_property(
            self.sc.key, WorkmodelService.CONFIGURATION_APP_KEY
        )
        return conf

    def migrate_from_1(self, conf):
        # Save the current configuration into their corresponding services configuration
        hierarchy_conf = {"version": 1, "hierarchy": conf.raw["value"]["hierarchy"]}
        self.addon_jira.create_app_property(
            self.sc.key, HierarchyService.CONFIGURATION_APP_KEY, hierarchy_conf
        )
        business_time_conf = {
            "version": 1,
        ...
        self.addon_jira.create_app_property(
            self.sc.key, BusinessTimeService.CONFIGURATION_APP_KEY, business_time_conf
        )
        # Finally remove the old configuration and keep just the version for compatibility reasons
        conf.delete()
        self.addon_jira.create_app_property(
            self.sc.key, WorkmodelService.CONFIGURATION_APP_KEY, {"version": 2}
        )
        conf = self.addon_jira.app_property(
            self.sc.key, WorkmodelService.CONFIGURATION_APP_KEY
        )
    ...
    def default_configuration(self):
        self.addon_jira.create_app_property(
            self.sc.key,
            WorkmodelService.CONFIGURATION_APP_KEY,
            {"version": WorkmodelService.CONFIGURATION_VERSION},
        )
        conf = self.addon_jira.app_property(
            self.sc.key, HierarchyService.CONFIGURATION_APP_KEY
        )
        return conf
...
class HierarchyService(JiraService):
    ...
    def migrate_from_1(self, conf):
        ...
        self.addon_jira.create_app_property(
            self.sc.key, HierarchyService.CONFIGURATION_APP_KEY, new_conf
        )
        conf = self.addon_jira.app_property(
            self.sc.key, HierarchyService.CONFIGURATION_APP_KEY
        )
    ...
    def child_issues(
        self, issue, expand=None, extra_jql=None, skip_levels=None, recurse=True
    ):
            ...
            for idx in range(0, len(h.hierarchies) - 1):
                ...
                if l.check_issue_type(issue):
                    ...
                    if l in already_calculated:
                        logger.debug(
                            "Hierarchy {} already processed, skipping it".format(l)
                        )
                        break
                    ...
                    for ch in children:
                        ...
                        if recurse:
                            for sch in self.child_issues(
                                ...
                                expand=expand,
                                extra_jql=extra_jql,
                                skip_levels=already_calculated,
                                recurse=recurse,
                            ):
                                yield sch
...
class BusinessTimeService(JiraService):
    ...
    def configuration_1(self):
        conf = self.addon_jira.app_property(
            self.sc.key, BusinessTimeService.CONFIGURATION_APP_KEY
        )
        return conf
    ...
    def transition_to_days(self, from_transition, to_transition):
        ...
        # Get the timespan
        daygenerator = (
            fromDate + datetime.timedelta(x + 1)
    ...
    def generate_transition_categories(self, issue):
        ...
        for h in issue.changelog.histories:
            ...
            for item in h.items:
                ...
                fromStatusList = [
                    x for x in self.statuses if x.id == getattr(item, "from")
                ]
                toStatusList = [x for x in self.statuses if x.id == getattr(item, "to")]
                # Try by name
                if not fromStatusList:
                    fromStatusList = [
                        x for x in self.statuses if x.name == item.fromString
                    ]
                if not toStatusList:
                ...
                try:
                    ...
                    logger.error(
                        "Status id: {} name: {} does not exist for issue {}".format(
                            getattr(item, "from"), item.fromString, issue
                        )
                    )
                    continue
        ...
        if transitions:
            ...
            if diff.days != 0:
                if fromStatus.statusCategory.name != last.status:
                    t = Transition(
                        abs_start, last.from_date, fromStatus.statusCategory.name
                    )
                    transitions.append(t)
                else:
    ...
    def business_time(self, issue):
        ...
        for h in self.hierarchy.hierarchies:
            ...
            if l.is_container and not has_children:
                ...
                children = self.hierarchy.child_issues(
                    issue, expand="changelog", recurse=False
                )
                for ch in children:
                    has_children = True
        ...
        if len(all_transitions):
            logger.info(
                "Normalizing {} transitions for issue {}: {}".format(
                    len(all_transitions), issue, all_transitions
                )
            )
            # Fake transitions
        ...
        days = sum(
            t.to_days() for t in transitions if t.status == Transition.IN_PROGRESS
        )
        logger.info("Days in progress for {} is {}".format(issue, days))

        self.jira.add_issue_property(
            issue.key, "transitions", {"progress_summation": days, "statuses": statuses}
        )
        return transitions
    ...
    def update_in_progress_business_time(self):
        ...
        issues = self.jira.search_issues_gen(
            "statusCategory = 'In Progress' AND type IN ({})".format(
                ",".join("'{0}'".format(it) for it in issue_types)
            )
        )
        to_update = []
        for i in issues:
            for h in self.hierarchy.hierarchies:
                try:
                    ...
                    continue
                logger.info(
                    "Updating business time for In-Progress issue {}".format(root.key)
                )
