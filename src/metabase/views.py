...
def jira_configuration(request):
    return render(request, "metabase/jira_configuration.html")
...
class ConfluenceConfiguration(View):
    ...
    def get(self, request):
        conf, created = MetabaseConfiguration.objects.get_or_create(
            account=request.atlassian_sc
        )
        if created:
            conf.save()
...
def macro_view(request):
    ...
    url = "/rest/api/content/{}/history/{}/macro/id/{}".format(
        page_id, page_version, macro_id
    )
    ...
    if response:
        try:
            ...
            if conf["payload"]["resource"].has_key("question"):
                ...
                resource = "dashboard"
            conf["payload"]["exp"] = round(time.time()) + (
                60 * 10
            ...
            return render(request, "metabase/confluence_wrong_configuration.html")
    else:
        return render(request, "metabase/macro_missing.html")
    return render(request, "metabase/macro_view.html", {"iframe_url": iframe_url})
...
def dashboard_item_view(request):
    ...
    configuration = [
        p
        for p in j.app_properties("com.fluendo.atlassian-addon")
        if p.key == "metabase-configuration"
    ]

    if not configuration:
        return render(request, "metabase/dashboard_item_missing_configuration.html")
    iframe_url = ""
    if dashboard_id and item_id:
        try:
            prop = j.dashboard_item_property(
                dashboard_id, item_id, "metabase-configuration"
            )
            if hasattr(prop.value.payload.resource, "question"):
                ...
                resource = "dashboard"
            payload = {
                "resource": {resource: getattr(prop.value.payload.resource, resource)},
                "params": getattr(prop.value.payload, "params", {}),
                "exp": round(time.time()) + (60 * 10),  # 10 minute expiration
            }
            token = jwt.encode(
                payload, configuration[0].value.secret_key, algorithm="HS256"
            )
            iframe_url = "{}/embed/{}/{}#bordered={}&titled={}&theme={}".format(
    ...
    return render(
        ...
        {"dashboard": dashboard_id, "dashboardItem": item_id, "iframe_url": iframe_url},
    )
